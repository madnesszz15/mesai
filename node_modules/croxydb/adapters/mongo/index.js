        ...
        if(!data) {
          ...
          if(db.includes(".")) {
            ...
            if (!content) {
              ...
              await this.schema.findOneAndUpdate({key: db.split(".").shift()}, { value: content}, {upsert: true});
              ...
              await this.schema.findOneAndUpdate({key: db.split(".").shift()}, {value: prev}, {upsert: true});
              return data;
            }
          } else {
  ...
  async delete(db) {
    if(!db) {
      ...
      if(db.includes(".")) {
        ...
        setTimeout(async() => {
        var newData = await this.get(db.split(".").slice(0, db.split(".").length - 1).join("."))
          if(typeof newData === "object") {
            if(Object.keys(newData).length === 0) {
              await this.delete(db.split(".").slice(0, db.split(".").length - 1).join("."))
            }
          }
        }, 500)
      } else {
